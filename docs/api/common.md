...
| 404 | Not Found | 리소스 없음, 일부 도메인의 소유권 실패 위장 |
...
| 인증은 되었지만 작업 권한이 없음 | 403 또는 404 | 도메인 정책에 따라 선택 |
| 리소스가 없거나 존재를 숨겨야 함 | 404 | 단가 제보 프로젝트명 수정 시 소유권 실패도 404 |
| 중복 이메일 등 제약 충돌 | 409 | `DuplicateValueException`, `DataIntegrityViolationException` |

403과 404의 선택은 전역 규칙만으로 결정하지 않는다. 리소스 존재 자체를 숨겨야 하는 API는 404를 사용할 수 있고, 명시적인 권한 부족을 표현해야 하는 API는 403을 사용할 수 있다.
...
운영 환경에서도 동일하게 적용되므로, 운영 도메인만 허용하도록 분리하는 작업은 개선 대상으로 관리한다.
